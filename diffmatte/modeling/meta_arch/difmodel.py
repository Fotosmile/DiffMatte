...
class DifModel(nn.Module):
    ...
    def forward(self, batched_inputs, features=None, intermidate_out=False):
        images, dimages, timesteps, H, W = self.preprocess_inputs(batched_inputs,
                                                                  decoder_in_channel=self.decoder_in_channel)
    ...
    def preprocess_inputs(self, batched_inputs, decoder_in_channel=7, L=320):
        ...
        if images.shape[-1] % 32 != 0 or images.shape[-2] % 32 != 0:
            ...
            new_images = torch.zeros((images.shape[0], images.shape[1], new_H, new_W), dtype=images.dtype,
                                     device=self.device)
            new_images[:, :, :H, :W] = images[:, :, :, :]

            dnew_images = torch.zeros((dimages.shape[0], dimages.shape[1], new_H, new_W), dtype=images.dtype,
                                      device=self.device)
